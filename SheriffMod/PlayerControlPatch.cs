...
namespace SheriffMod
{
	...
	public static class PlayerControlPatch
	{
		...
		public static List<FFGALNAPKCD> dunce = new List<FFGALNAPKCD>();
				...
				case (byte)CustomRPC.SetDunce:
                    {
						...
						foreach (FFGALNAPKCD ffgalnapkcd in FFGALNAPKCD.AllPlayerControls)
						{
							...
							if (flag)
							{
								PlayerControlPatch.dunce.Add(ffgalnapkcd);
								if (CustomGameOptions.permOn && FFGALNAPKCD.LocalPlayer.NDGFFHMFGIG.DAPKNDBLKIA)
								{
									ffgalnapkcd.nameText.Color = new Color(1f, 0f, 1f, 0f);
								}
							}
						}
						break;
		...
		public static bool isDunce(FFGALNAPKCD player)
		{
			return isDunce(player.PlayerId);
		}
		public static bool isDunce(byte playerId)
		{
			return PlayerControlPatch.dunce.Select(player => player.PlayerId).Contains(playerId);
		}
		...
		//change to impostors
		public static List<FFGALNAPKCD> getImpostors(Il2CppReferenceArray<EGLJNOMOGNP.DCJMABDDJCF> impostors)
		{
			List<FFGALNAPKCD> list = new List<FFGALNAPKCD>();
			foreach (FFGALNAPKCD teammate in FFGALNAPKCD.AllPlayerControls)
			{
				if (impostors.Select(player => player.LAOEJKHLKAI.PlayerId).Contains(teammate.PlayerId))
				{
					list.Add(teammate);
				}
			}
		...
		public static void Postfix(Il2CppReferenceArray<EGLJNOMOGNP.DCJMABDDJCF> JPGEIBIBJPJ)
		{
			...
			MessageWriter messageWriter2 = FMLLKEACGIO.Instance.StartRpcImmediately(FFGALNAPKCD.LocalPlayer.NetId, 40, 0, -1);
			List<FFGALNAPKCD> impostorList = PlayerControlPatch.getImpostors(JPGEIBIBJPJ);
			for (int i = 0; i < PlayerControlPatch.getImpostors(JPGEIBIBJPJ).Count ; i++)
			{
				FFGALNAPKCD sheriff = impostorList[UnityEngine.Random.Range(0, impostorList.Count)];
				PlayerControlPatch.dunce.Add(sheriff);
				impostorList.Remove(sheriff);
				byte duncePlayerId = sheriff.PlayerId;
				messageWriter2.Write(duncePlayerId);
		...
		public static void Postfix(KMOGFLPJLLK IOFBPLNIJIC)
		{
			if (FMLLKEACGIO.Instance != null && FFGALNAPKCD.LocalPlayer != null)
			{
				...
				messageWriter.Write(CustomGameOptions.permOn);
				FMLLKEACGIO.Instance.FinishRpcImmediately(messageWriter);
			}
		}
	}
