...
namespace SheriffMod
{
	...
	public static class GameOptionMenuPatchStart
	{
		...
		public static void Postfix(PHCKLDDNJNP __instance)
		{
			...
			if (flag)
			{
				BCLDBBKFJPK bcldbbkfjpk2 = (from x in UnityEngine.Object.FindObjectsOfType<BCLDBBKFJPK>().ToList<BCLDBBKFJPK>()
											where x.TitleText.Text == "Confirm Ejects"
											select x).First<BCLDBBKFJPK>();
				// Create permutations label
				GameOptionMenuPatchUpdate.permOn = UnityEngine.Object.Instantiate<BCLDBBKFJPK>(bcldbbkfjpk2);
				GameOptionMenuPatchUpdate.permOn.TitleText.Text = "Permutations";
				GameOptionMenuPatchUpdate.permOn.NHLMDAOEOAE = CustomGameOptions.permOn;
				GameOptionMenuPatchUpdate.permOn.CheckMark.enabled = CustomGameOptions.permOn;
				...
				LLKOLCLGCBD[] array = new LLKOLCLGCBD[__instance.KJFHAPEDEBH.Count + 4];
				__instance.KJFHAPEDEBH.ToArray<LLKOLCLGCBD>().CopyTo(array, 0);
				array[array.Length - 4] = GameOptionMenuPatchUpdate.permOn;
